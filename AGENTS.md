...
Adding a benchmark: register it in `benchmark-registry.json`, declare it in the service's `BENCHMARKS` mapping (dimensions and type are cross-checked against the registry), and record a golden if it is deterministic — `pytest tests --update-goldens` regenerates them, merging rather than overwriting. Stochastic benchmarks take a `seed`; pin their goldens at a fixed seed rather than leaving them unchecked. Mark anything needing a baked-in dataset `@pytest.mark.dataset` so it only runs where the data exists.

Goldens live in `<Package>/tests/goldens.json`, sectioned by platform: `any` for values that reproduce anywhere, and a `system-machine` key such as `linux-x86_64` for ones that do not. Only MuJoCo needs the latter — physics rollouts diverge across architectures, up to a sign flip. A package opts in with `GOLDENS_PLATFORM_SPECIFIC = True` in its conftest; otherwise record under `any`. Where the running platform has no recorded value the golden test skips and the relative determinism assertions still run. To capture values for an architecture you are not on, run the `record-goldens` workflow_dispatch input and commit the uploaded artifact — do not record under emulation.
